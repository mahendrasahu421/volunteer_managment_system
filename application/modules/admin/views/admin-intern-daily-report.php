		...
		<div class="main-container container-fluid">
			...
			<div class="page-header">
				...
				<div>
					...
					<h1 class="page-title">Approve Daily Report ( Interns )</h1>
			...
			<div class="row row-sm">
				...
				<div class="col-lg-12">
					...
					<div class="card">
						...
						<form action="admin-intern-daily-report" method="post">
								...
								<div class="col-md-3">
									...
									<select class="form-control select2-show-search form-select" name="task_id">
										...
										?>
											...
											<option value="<?php echo $value['intern_task_id']; ?>" <?php echo $task_id == $value['intern_task_id'] ? 'selected' : ''; ?>><?php echo ucwords($value['task_title']); ?></option>
							...
							<div class="table-responsive">
								...
								<table id="example" class="display nowrap" style="width:100%">
									...
									<tbody>
										...
										foreach ($internDailyReport as $internData) {

											$intern_id = $internData['intern_id'];

											$encode_taskID = rtrim(strtr(base64_encode($internData['intern_task_id']), "+/", "-_"), "=");

											$encode_userID = rtrim(strtr(base64_encode($internData['intern_id']), "+/", "-_"), "=");
											...
											$timeIn = $internData['dr_time_in'];

											$time = date('h:i A', strtotime($timeIn));

											$timeOut = $internData['dr_time_out'];
											...
											<tr>
												...
												<td class="text-light-blue"><a href="#" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="fa fa-ellipsis-v"></i></a>
													...
													<div class="dropdown-menu">
														...
														<a onclick="return confirm('Do you want to approved'); " href="<?php echo base_url(); ?>dailyreport-approved/<?php echo $encoded_id; ?>/<?php echo $encode_taskID; ?>/<?php echo $total_time; ?>/<?php echo $internData['dr_date']; ?>" class="dropdown-item" href="#">Approve</a>

														<a href="#" data-toggle="modal" data-target=".project-details" class="dropdown-item" onclick="disapproved('<?php echo $internData['intern_id']; ?>','<?php echo $internData['task_id']; ?>','<?php echo $total_time; ?>','<?php echo $internData['dr_date']; ?>');">DisApprove</a>
...
<script>
	...
	function disapproved(volunteer_id, task_id, time) {
		...
		$('#intern_id').val(volunteer_id);

		$('#intern_task_id').val(task_id);
...
<script>
	...
	function fetch_details(id, userid,  display_id) {
		...
		var request = $.ajax({
			...
			url: '<?php echo base_url("fetch-daily-report-intern"); ?>',
