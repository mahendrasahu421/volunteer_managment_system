		...
		<div class="main-container container-fluid">
			...
			<div class="col-md-12">
				<div class="card">
					...
					<form action="<?php echo base_url(); ?>intern_update_task" method="post" id="form" name="pForm" onsubmit=" return validate();" enctype="multipart/form-data" class="needs-validation" novalidate>
						<div class="card-body">
							<div class="row">
								<input name="intern_task_id" type="hidden" class="form-control" value="<?php echo $interntaskData[0]['intern_task_id']; ?>" required>
								<div class="col-md-4">
									...
									<select class="form-control select2-show-search form-select" name="task_type_id" id="validationCustom04" required>
										...
										<?php foreach ($task_type as $tt) { ?>
											<option value="<?php echo $tt['task_type_id']; ?>" <?php if ($interntaskData[0]['task_type_id'] == $tt['task_type_id']) {
																									echo  "selected";
								...
								<div class="col-md-4">
									...
									<select class="form-control select2-show-search form-select" name="region_id" id="region_id" required>
										...
										<?php foreach ($regions as $rd) { ?>
											<option value="<?php echo $rd['region_id']; ?>" <?php if ($interntaskData[0]['region_id'] == $rd['region_id']) {
																								echo  "selected";
								...
								<div class="col-md-4 state">
									...
									<select class="form-control select2-show-search form-select" name="state_name" id="state_name" required>
										...
										<?php foreach ($states as $sd) { ?>
											<option value="<?php echo $sd['state_id']; ?>" <?php if ($interntaskData[0]['task_state_id'] == $sd['state_id']) {
																								echo  "selected";
								...
								<div class="col-md-4 state">
									<label for="validationCustom02" class="form-label">Select City<sup class="text-danger fs-6">*</sup></label>
									<select class="form-control select2-show-search form-select" name="city" id="city" required>
										<option>Select City</option>
										<?php foreach ($cities as $cd) { ?>
											<option value="<?php echo $cd['city_id']; ?>" <?php if ($interntaskData[0]['city_id'] == $cd['city_id']) {
																								echo "selected";
																							} ?>>
								...
								<div class="col-md-4">
									...
									<input type="date" name="sdate" id="sdate" value="<?php echo $interntaskData[0]['start_date']; ?>" onkeyup="onlyNumber('sdate');" class="form-control dob_caf" required />
									<div class="ininvalid-feedback"></div>
								</div>
								<div class="col-md-4 ">
									<label for="validationCustom02" class="form-label">Expected End Date <sup>*</sup></label>
									<input type="date" name="edate" id="edate" onkeyup="onlyNumber('sdate');" value="<?php echo $interntaskData[0]['expected_end_date']; ?>" class="form-control dob_caf" required />
									<div id="expdateerror" class="col-md-12"></div>
									<div class="invalid-feedback"></div>
								</div>
								<div class="col-md-4">
									<label for="validationCustom02" class="form-label">Total Intern Required <sup>*</sup></label>
									<input type="text" class="form-control" onkeyup="onlyNumber('volunteer');" id="volunteer" name="intern_required" value="<?php echo $interntaskData[0]['intern_required']; ?>" required />
								</div>
								<div class="col-md-4">
									...
									<select class="form-control select2 form-select" name="keywords[]" multiple required id="keywords">
										...
										<?php $selectedSkill = explode(',', $interntaskData[0]['keyword']); ?>
										<?php foreach ($skills as $skillsData) { ?>
											<option value="<?php echo $skillsData['skill_id']; ?>" <?php echo in_array($skillsData['skill_id'], $selectedSkill) ? "selected" : ""; ?>>
								...
								<div class="col-md-4">
									...
									<textarea class="form-control" value="" id="exampleFormControlTextarea2" rows="2" placeholder="Title" maxlength="150" name="title" required><?php echo $interntaskData[0]['task_title']; ?></textarea>
									<div class="invalid-feedback"></div>
								</div>
								...
								<div class="col-md-12">
									...
									<div class="">
										<textarea class="content" name="what_to_do" rows="4" cols="10" required><?php echo $interntaskData[0]['task_brief']; ?></textarea>
									</div>
								</div>
								<div class="col-md-6 upstair">
									...
									<select class="form-select select2 form-control" name="status" id="validationCustom04" required>
										...
										<option value="1" <?php if ($interntaskData[0]['status'] == 1) {
																echo  "selected";
															} ?>>
											Published
										</option>
										<option value="2" <?php if ($interntaskData[0]['status'] == 2) {
																echo  "selected";
															} ?>>
											Unpublished
										</option>
