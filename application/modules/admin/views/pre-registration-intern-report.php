                        ...
                        <form action="<?php echo base_url() ?>pre-registration-intern-report" method="post">
                            ...
                            <div class="card-header">
                                ...
                                <div class="col-md-8"></div>
                                 <div class="col-md-2">
                                    <div class="form-group">
                                        <a href="<?php echo base_url()?>export-all-pre-registration-intern-report"><button type="button" id="exportAlll" class="btn btn-success form-control">Export All</i></button></a>                                    <textarea id="allData" style="display:none;"><?php echo base64_encode(json_encode($allRecord, JSON_UNESCAPED_UNICODE)); ?></textarea>
									
 <textarea id="selectedIntern" style="display:none;"><?php echo base64_encode(json_encode($selectedIntern, JSON_UNESCAPED_UNICODE)); ?></textarea>
...
<script>
   $('#export').on('click', function (e) {
    ...
    e.preventDefault();
    let encodedData = $("#allData").val();
    let jsonString = atob(encodedData);  // decode base64 string
    try {
        let data = JSON.parse(jsonString);
        JSONToCSVConvertor(data, "All Interns Data", true);
    } catch (e) {
        console.error("Invalid JSON string", e);
        alert("Data export failed due to invalid data format.");
    }
