...
class Curl_model extends CI_Model{
   ...
   public function fetch_single_data($fields,$tbl_name,$where)
	...
	public function dioceses_list($lastID){
		...
		$query = $this->db->query("SELECT d.*, COUNT(u.dioceses_id) as tv FROM `dioceses` d LEFT JOIN `user_data` u ON d.dioceses_id = u.dioceses_id AND d.status = 1 GROUP BY d.dioceses_id order by tv DESC LIMIT $lastID,10");
	...
	public function referral_mail($email,$name_of_honer,$link)
	{
		try {
			...
			$mail = new PHPMailer();
			$to =$email;
			$subject = $this->session->userdata('name_user').' referred you - Join Devjoin!';
			...
			$mail->IsSMTP();
            $mail->Host = "mail.devjoin.in";
			$mail->SMTPAuth = true;
			$mail->SMTPSecure = 'tls';
			$mail->SMTPDebug = 1; // debugging: 1 = errors and messages, 2 = messages only
			$mail->Port = 587;
			$mail->Username = "support@devjoin.in";
			$mail->Password = "devjoin!@#$";
			$mail->setFrom('pransi.g@neuralinfo.org');
			$mail->AddAddress($to);
