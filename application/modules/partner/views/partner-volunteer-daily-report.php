...
<div class="row ml-3">
	...
	<div class="col-md-4">
		...
		<div class="card-body">
			<input type="text" name="asdate" required class="form-control dob_caf" value="<?php if(isset($birthday)){echo date("d-m-Y", strtotime($birthday)); } ?>" />
		</div>
	</div>
	<div class="col-md-2">
	<div class="card-header">
...
<td><?php if($value['gender']==1){ echo "Mr."; }elseif($value['gender']==2){ echo "Mrs.";}?> <?php echo ucwords($value['firstName'].' '.$value['lastName']); ?> <br><a href="#" data-toggle="modal" data-target=".daily-report" onclick="fetch_details('<?php echo $encode_taskID; ?>','<?php echo $encode_userID; ?>','<?php echo $value['dailyReportDate']; ?>','daily-report');"><small class="text-primary">(View details)</small></a></td>
...
<script>
	function fetch_details(id,userid,date,display_id)
	{
        ...
        method: "POST",
        data: {taskID : id, userID : userid, dailyReportDate: date},
